...
Secara default aplikasi memakai **Asia/Jakarta (WIB)** — `date_default_timezone_set('Asia/Jakarta')` di `config.php`.
Sesuaikan dengan zona waktu lokasi usaha (contoh untuk Samarinda: `Asia/Makassar`) di `config.php`,
dan pastikan zona waktu OS sama:

```bash
sudo timedatectl set-timezone Asia/Jakarta
```

> Catatan: pada varian Rainbow (produksi) zona waktu sudah WITA (Asia/Makassar) dan
> riwayat data lama telah disesuaikan pada proses migrasi (11 Agu 2026).
...
### Pasang library PDF (opsional, untuk download nota/rekap PDF)

```bash
sudo mkdir -p /var/www/kasir-lib
cd /var/www/kasir-lib
curl -L https://github.com/dompdf/dompdf/releases/download/3.1.6/dompdf-3.1.6.zip -o dompdf.zip
unzip -o dompdf.zip -d .
```
Tanpa library ini, halaman nota/rekap tetap jalan (HTML print), hanya tombol download PDF yang menampilkan pesan "Library PDF belum terpasang.".
